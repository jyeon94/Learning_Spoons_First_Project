...
df_cat[str_detect(df_cat$품종,'^[0-9]|화이트오리지널|기카'),"품종"] <- "기타"
df_cat[str_detect(df_cat$품종,'혼종'),"품종"] <- "믹스"
df_cat[str_detect(df_cat$품종,'아메리칸컬헤어'),"품종"] <- "아메리칸컬"
df_cat[str_detect(df_cat$품종,'믹스'),"품종"] <- "믹스"
df_cat[str_detect(df_cat$품종,'고등어|길양이|삼색이|젖소|카오스|턱시도'),"품종"]<-"한국고양이" 
df_cat[str_detect(df_cat$품종,'친칠라'),"품종"]<-"페르시안" 
...
view(table(df_cat$품종))

write_csv(df_cat,'csv/품종(고양이).csv')
# 개
df_dog <- df %>%
    filter(종류=='개')
...
df_dog[str_detect(df_dog$품종, removed), "품종"] <- "기타"
...
df_dog[str_detect(df_dog,"무트$")] <- "맬러뮤트"

dog59 <- c('브래타니|브레타니|브루태니|브리타니|브리타니스|브리티나|프렌치브리타니')
df_dog[str_detect(df_dog$품종,dog59),'품종']<-'브리트니'
df_dog[str_detect(df_dog$품종,"드툴|^꼬"),"품종"]<-"꼬통드툴레아"
df_dog[str_detect(df_dog$품종,"엘크"),"품종"]<-"노르웨이엘크하운드"
df_dog[str_detect(df_dog$품종,"노리치"),"품종"]<- "노리치테리어"
df_dog[str_detect(df_dog$품종,"노르포크테리어"),"품종"] <- "노퍽테리어"
df_dog[str_detect(df_dog$품종,"레드포인트"),"품종"] <- "기타"
df_dog[str_detect(df_dog$품종,"뉴펀"),"품종"] <- "뉴펀들랜드"
df_dog[str_detect(df_dog$품종,"달마"),"품종"] <- "달마시안"
df_dog[str_detect(df_dog$품종,"아르젠티노"),"품종"] <- "도고아르헨티노"
df_dog[str_detect(df_dog$품종,"랜드테리어"),"품종"] <- "레이클랜드테리어"
df_dog[str_detect(df_dog$품종,"로트|롯트"),"품종"] <- '로트바일러'
df_dog[str_detect(df_dog$품종,"리트리버종|리트리버"),"품종"] <- '리트리버'
df_dog[str_detect(df_dog$품종,"^마스티"),"품종"] <- "마스티프"
df_dog[str_detect(df_dog$품종,"뮤트$"),"품종"] <- "맬러뮤트"
df_dog[str_detect(df_dog$품종,"말티츄"),"품종"] <- "말티즈"
df_dog[str_detect(df_dog$품종,"말티푸"),"품종"] <- "말티푸"
df_dog[str_detect(df_dog$품종,"^맨체"),"품종"] <- "맨체스터테리어"
df_dog[str_detect(df_dog$품종,"미니핀"),"품종"] <- "미니어쳐핀셔"
df_dog[str_detect(df_dog$품종,"스견$|백구|잡견|교잡|밀러"),"품종"] <- "믹스견"
df_dog[str_detect(df_dog$품종,"브르"),"품종"] <- "보르조이"
df_dog[str_detect(df_dog$품종,"사모예드|샤모예드|샤모애드"),"품종"]<-"사모예드"
...
df_dog[str_detect(df_dog$품종, str72), "품종"] <- "카이오트"
df_dog[str_detect(df_dog$품종, str73), "품종"] <- "코리아트라이하운드"
df_dog[str_detect(df_dog$품종, str74), "품종"] <- "티베탄테리어"
df_dog[str_detect(df_dog$품종, str75), "품종"] <- "페키니즈"
df_dog[str_detect(df_dog$품종, str76), "품종"] <- "포메라니안"
df_dog[str_detect(df_dog$품종, str77), "품종"] <- "폭스하운드"
df_dog[str_detect(df_dog$품종, str78), "품종"] <- "하바나실크독"
df_dog[df_dog$품종=='강아지',"품종"] <- "기타"
view(df_dog[str_detect(df_dog$품종,""),"품종"])


z<-data.frame(table(df_dog$품종))

view(table(df_dog$품종))

df_dog[df_dog$품종=="","품종"] <-"기타"

write_csv(df_dog,'csv/품종(개).csv')
