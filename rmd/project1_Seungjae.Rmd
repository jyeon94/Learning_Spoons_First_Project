...
* 인터넷 뉴스에서는 유기 동물 수 증가의 원인으로 1인 가구 수의 증가를 꼽고 있다. 1인 가구가 증가하면서 반려 동물의 입양이 늘었으며 유기 또한 증가하였다는 것이다. 정부는 무책임한 동물 유기에 대한 방안으로 **반려 동물에 대한 인식 캠페인**을 수행과 동물 보호 및 복지를 위한 *예산*을 편성하여 유기 동물 보호소를 지원하고 입양을 권장
...
-----------|-------------|----------|-----------------------
동물보호관리시스템 유기동물 조회 서비스 | 접수일자, 품종, 발견장소, 상태, 관할기관 | 연 단위 | 농림축산식품부 | Open API 
...
* 유기 동물 조회 서비스 시작인 2014년부터 서비스 개편 이전인 2016년까지의 데이터는 입력 기준이 ***존재 하지 않음***

* 2014년 ~ 2016년 문자 전처리 진행
* 2017년 ~ 2019년 OPEN API 추출
### 사용 패키지
```{r, eval=F}
library(tidyverse)
library(XML)
...
#xml을 파싱하기 위해 urlist에 있는 빈 리스트 생성
raw.data <- list()
rootnode <- list()
for(i in 1:length(urlist)){
  #xml 파싱
  # XML 을 요청하기 위해 생성한 URL을 적용
...
#접수일, 품종, 발견장소,공고번호,상태 데이터를 담기 위해 빈 벡터 생성
a <- c()
b <- c()
c <- c()
d <- c()
...
#union_all을 하지 않으면 전에 삽입했던 값이 사리지고 union을사용하면 중복된 값이 삭제 가능성이 높음
...
# csv 파일로 저장
write.csv(df,'./csv/abandoned_animal_national_ver3.csv',row.names = FALSE)
```
...
# "|"의 다른 뜻인 OR을 뜻하기 때문에 모든값을 확인하기 위해서 ")"으로 변경
...
total5 <- total4[total4$years!=2015]
```
...
* 2015년부터 2017년까지 연평균 약 20% 유기 동물 수 증가
* 특히 2017년, 2018년은 유기 동물 수가 약 28% 증가
...
```{r, include=F, message=F}
# CSV파일안에 있는 데이터와 혼란을 줄 수 있는 NA값 제거
df_animal <- read.csv("./data/abandoned_animal(All)_ver2.csv")
df_animal$접수일 <- ymd(df_animal$접수일)
df_animal <- df_animal %>%
    drop_na()
...
```{r}
# 청자의 이해를 돕기 위해서 전체 유기동물 BAR CHART 애니메이션 수행
...
```{r, echo=F}
# R markdown에 표를 보기 위해서 저장
image_read("./output/연도별 전체 유기동물 수.gif")
```

## 연도별 유기견, 유기묘 현황

* 유기견은 연평균 약 24% 증가, 유기묘는 연평균 약 11% 증가
* 유기견 증가율은 2017년, 2018년에 각각 31%, 44%로 큰 폭으로 상승
* 유기묘는 증가율은 2016년에 17%로 크게 상승
* 2019년, 유기견 증가율은 약 12%로 둔화되었으나 유기묘 증가율은 15%로 늘어나고 있음
...
* 1인 가구 비율의 전국 평균은 약 29%로 경기도, 울산광역시, 인천광역시의 1인 가구 비율은 약 25%로 나타남
* 1인 가구 수는 경기도 > 서울특별시 > 부산광역시 > 경상남도 순으로 나타남
* 1인 가구 증가율은 세종특별시 > 제주특별시 > 경기도 순으로 증가
...
```{r}
# 1인 가구 비율
gg1 <- ggplot(total4, aes(x=total4$행정구역별, y=years, fill=percent))
...
# 1인 가구 수
gg2 <- ggplot(total4, aes(x=total4$행정구역별, y=years, fill=total4$one_person)) 
...
# 1인 가구 증가율
gg3 <- ggplot(total4, aes(x=total4$행정구역별, y=years, fill=increase)) 
...
### 경기도 1인 가구 수와 유기 동물 간 상관관계 분석

```{r, include=F, message=F}
# 경기도 내 시별 인구 수 및 1인 가구 수와 유기 동물 수 결합
...
```{r, echo=F, warning=F}
# 시각화
gyungi_cor <- ggplot(df_merged, aes(one_house,유기동물수)) +
  geom_point(colour = 'black') +
  theme_classic(base_family = "NanumBarunGothic") +
  theme(plot.title = element_text(family = "NanumBarunGothic", hjust = 0.5)) +
  labs(x = "1인 가구 수", y = "유기 동물 수", title = '경기도') +
  xlim(0,100000) +
...
df_pops_seoul_one <- subset(df_pops_seoul_one, select= -c(2,3,4,5))
...
### 서울특별시 1인 가구 수와 유기 동물 간 상관관계 분석

```{r, include=F, message=F}
# 서울특별시 내 구별 인구 수 및 1인 가구 수와 유기 동물 수 결합
...
seoul_cor <- ggplot(df_seoul_merged, aes(one_house,유기동물수)) +
  ...
  theme_classic(base_family = "NanumBarunGothic") +
  theme(plot.title = element_text(family = "NanumBarunGothic", hjust = 0.5)) +
  labs(x = "1인 가구 수", y = "유기 동물 수", title = '서울시') +
  xlim(0,100000) +
  stat_smooth(method=lm,col='red',geom='smooth',se=FALSE) 
```
...
## 가설 검증 결과
* 1인 가구 수와 유기 동물 증가의 상관관계는 0.5563873로 연관이 있다고 보여진다.
...
* 서울시, 경기도를 비롯한 지자체에서 동물 보호 정책을 발표, 사업을 운영하고 있다. 
* 사업 주요 내용은 이미 발생된 유기 동물에 대한 보호 및 입양이다.
* 유기 동물 방지를 위해 가장 선행되어야 할 것은 무분별한 동물 판매 및 입양을 막는 것이 되어야 할 것이다. 
